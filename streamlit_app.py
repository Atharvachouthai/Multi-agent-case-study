...
#                 last_graph_msg = graph_messages_history[-1] if graph_messages_history else None # Check user's last message in this turn's context
                
#                 # To find what the AI said when it planned tools, we need to look at the AIMessage that had these tool_calls_for_this_turn
#                 # This is tricky as it's already passed in the stream. We can get it from the final_graph_state if it's the last message
#                 # or from the accumulated tool_calls_for_this_turn.
#                 # The variable `agent_response_content` in previous iterations was accumulating AIMessage.content during the stream.
#                 # For simplicity, if the turn ends in tool calls, we use a generic message or the LLM's last utterance if available.
#                 # Check the last actual message from the graph for this turn
#                 if final_graph_state_after_turn and final_graph_state_after_turn.get("messages"):
#                     last_msg_in_turn_for_content = final_graph_state_after_turn["messages"][-1]
#                     if isinstance(last_msg_in_turn_for_content, AIMessage) and last_msg_in_turn_for_content.tool_calls and last_msg_in_turn_for_content.content:
#                          temp_ai_content_with_tool_call = last_msg_in_turn_for_content.content
